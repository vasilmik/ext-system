...
@NamedQueries(value = {
        @NamedQuery(name = "findPersons",
                query = "SELECT p FROM Person p LEFT JOIN fetch p.passports ps LEFT JOIN FETCH p.birthCertificate bs"),
        @NamedQuery(name = "findPerson",
                query = "SELECT p FROM Person p LEFT JOIN fetch p.passports ps LEFT JOIN FETCH p.birthCertificate bs WHERE p.id = :PersonID")
        }
)
